        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Sửa ngành</h4>
                </div>
                <div class="card-body">
                    @foreach($edit_nganh as $key => $edit_value)
                    <form action="{{URL::to('/update-nganh/'.$edit_value->idnganh)}}" method="POST">
                    @csrf   
                        ...
                        <div class="form-group">
                            ...
                            @foreach($all_khoa as $key => $value)
                                @if($value->idkhoa==$edit_value->idkhoa)
                                <option selected value="{{ $value->idkhoa }}">{{$value->tenkhoa}}</option>
                                @else
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="tennganh" id="tennganh" value="{{ $edit_value->tennganh }}" placeholder="Nhập ngành cần thêm...">
                        </div>
                        <div>
